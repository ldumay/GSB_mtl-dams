...
function obtenirReqMoisFicheFraisNonValide($unIdVisiteur) {
    $req = "select fichefrais.mois as mois from  fichefrais where fichefrais.idvisiteur ='"
 ...
 */
function verifierInfosConnexion($idCnx, $unLogin, $unMdp, $typeUser) {
    $unLogin = filtrerChainePourBD($unLogin);
    $unMdp = filtrerChainePourBD($unMdp);
    // le mot de passe est crypté dans la base avec la fonction de hachage md5
    $req = "select id, nom, prenom, login, mdp from ".$typeUser." where login='".$unLogin."' and mdp='" . $unMdp . "'";
